...
export const getNftUserWalletToList = async (userWalletAddress: PublicKey, connection: Connection, symbol: string) => {
    ...
    const nftListFiltered = walletNftArray.filter(item => item.data.symbol === symbol);
