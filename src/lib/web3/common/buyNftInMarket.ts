...
export const buyNftInMarket = async (wallet: any, nft: NftInMarketplace, connection: Connection, IDL: any, PROGRAM_ID: String, amount: number, REWARD_TOKEN: string) => {
    try {
        ...
        const response = await sendTransactions(connection, wallet, instructionSet, signerSet, SequenceType.StopOnFailure);
