...
export const listNftToSell = async (nft: NftToList, userWallet: any, connection: Connection, IDL: any, PROGRAM_ID: string, REWARD_TOKEN_DECIMAL: number, amount: number) => {
    try {
        ...
        const nftFrom = await getAssociatedTokenAddress(new PublicKey(nft.mint), userWallet!.publicKey!);
        const nftToAtaTx = await makeATokenAccountTransaction(connection, userWallet!.publicKey, new PublicKey(vaultPDA), new PublicKey(nft.mint));
        ...
        await sendTransactions(connection, userWallet, instructionSet, signerSet, SequenceType.StopOnFailure);
